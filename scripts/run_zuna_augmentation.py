...
def main() -> int:
    ...
    input_dir = Path(args.input_fif_dir)
    with tempfile.TemporaryDirectory(prefix="zuna_input_") as tmp_input_dir:
        zuna_input_dir = input_dir
        if args.subject_id:
            matches = sorted(input_dir.glob(f"*{args.subject_id}*.fif"))
            if not matches:
                raise SystemExit(
                    f"No FIF files found in {input_dir} matching subject filter: {args.subject_id}"
                )
            zuna_input_dir = Path(tmp_input_dir)
            for src in matches:
                (zuna_input_dir / src.name).symlink_to(src.resolve())
            print(
                f"Subject filter {args.subject_id}: selected {len(matches)} file(s) for ZUNA input."
            )
